        ...
        <div className="case-modal" role="presentation" onMouseDown={onClose}>
            ...
            >
                <header className="case-modal__header">
                    ...
                    <button aria-label="Đóng" className="case-icon-button" disabled={!onClose} type="button"
                            onClick={onClose}>
                        <X size={18}/>
                    </button>
                </header>
        ...
        <CaseModal onClose={submitting ? undefined : onClose} title={isEditing ? "Chỉnh sửa hồ sơ" : "Thêm hồ sơ"}>
            <form onSubmit={handleSubmit}>
                <div className="case-modal__body case-form-grid">
                    ...
                    <label><span>Chủ hồ sơ</span><input autoFocus value={form.applicantName}
                                                        onChange={(event) => updateForm("applicantName", event.target.value)}/></label>
                    <label><span>Số điện thoại</span><input value={form.applicantPhone}
                                                            onChange={(event) => updateForm("applicantPhone", event.target.value)}/></label>
                    <label>
                        <span>Đơn vị</span>
                        <select
                    ...
                    <label><span>Trạng thái</span><select value={form.status}
                                                          onChange={(event) => updateForm("status", event.target.value)}>{caseStatuses.map((status) =>
                        <option key={status} value={status}>{status}</option>)}</select></label>
                    <label><span>Ngày tiếp nhận</span><input type="datetime-local" value={form.receivedAt}
                                                             onChange={(event) => updateForm("receivedAt", event.target.value)}/></label>
                    <label><span>Ngày hẹn trả</span><input type="datetime-local" value={form.appointmentDate}
                                                           onChange={(event) => updateForm("appointmentDate", event.target.value)}/></label>
                    <label><span>Hạn xử lý</span><input type="datetime-local" value={form.dueAt}
                                                        onChange={(event) => updateForm("dueAt", event.target.value)}/></label>
                    <label><span>Ưu tiên</span><input value={form.priority}
                                                      onChange={(event) => updateForm("priority", event.target.value)}/></label>
                    <label><span>Bước hiện tại</span><input value={form.currentStepName}
                                                            onChange={(event) => updateForm("currentStepName", event.target.value)}/></label>
                    {/*<label><span>Nguồn dữ liệu</span><input value={form.sourceType}*/}
                    {/*                                        onChange={(event) => updateForm("sourceType", event.target.value)}/></label>*/}
                    {catalogsError &&
                        <p className="case-form-error case-form-grid__wide" role="alert">{catalogsError}</p>}
                ...
                <footer className="case-modal__footer">
                    ...
                    <button className="cases-button cases-button--primary"
                            disabled={submitting || !catalogsReady || usersLoading || Boolean(usersError)}
                            type="submit">
...
function DeleteCaseModal({caseItem, deleting, error, onClose, onConfirm}) {
    return (
        <CaseModal onClose={deleting ? undefined : onClose} title="Xóa hồ sơ">
            ...
            <footer className="case-modal__footer">
                <button className="cases-button cases-button--secondary" disabled={deleting} type="button"
                        onClick={onClose}>Hủy
                </button>
                <button className="cases-button cases-button--danger" disabled={deleting} type="button"
                        onClick={onConfirm}>
                ...
                {!loading && !error && caseItem && (
                    <>
                        <dl className="case-details">
                            ...
                            <div className="case-details__item">
                                <dt>Số điện thoại</dt>
                                <dd>{caseItem.applicantPhone ?? "—"}</dd>
                            </div>
                            <div className="case-details__item">
                                <dt>Đơn vị</dt>
                                <dd>{caseItem.agencyName ?? "—"}</dd>
                            </div>
                            <div className="case-details__item case-details__item--wide">
                                <dt>Thủ tục hành chính</dt>
                                <dd>{caseItem.procedureName ?? "—"}</dd>
                            </div>
                            <div className="case-details__item">
                                <dt>Phòng ban</dt>
                                <dd>{caseItem.departmentName ?? "—"}</dd>
                            </div>
                            <div className="case-details__item">
                                <dt>Người xử lý</dt>
                                <dd>{caseItem.assigneeName ?? "—"}</dd>
                            </div>
                            <div className="case-details__item">
                                <dt>Ngày tiếp nhận</dt>
                                <dd>{formatDateTime(caseItem.receivedAt)}</dd>
                            </div>
                            <div className="case-details__item case-details__item--appointment">
                                <dt>Ngày hẹn trả</dt>
                                <dd>{formatDateTime(caseItem.appointmentDate)}</dd>
                            </div>
                            <div className="case-details__item">
                                <dt>Hạn xử lý</dt>
                                <dd>{formatDateTime(caseItem.dueAt)}</dd>
                            </div>
                            <div className="case-details__item">
                                <dt>Ngày hoàn thành</dt>
                                <dd>{formatDateTime(caseItem.completedAt)}</dd>
                            </div>
                            <div className="case-details__item">
                                <dt>Trạng thái</dt>
                            ...
                            <div className="case-details__item">
                                <dt>Bước hiện tại</dt>
                                <dd>{caseItem.currentStepName ?? "—"}</dd>
                            </div>
                            <div className="case-details__item">
                                <dt>Nguồn dữ liệu</dt>
                                <dd>{caseItem.sourceType ?? "—"}</dd>
                            </div>
                        </dl>
...
function CasesPage() {
    ...
    async function openEditCase(caseId) {
        try {
            ...
            setFeedback({
                type: "error",
                message: getApiErrorMessage(requestError, "Không thể tải hồ sơ để chỉnh sửa.")
            });
        } finally {
    ...
    return (
        <section className="cases-page">
            ...
            {feedback && (
                <div className={`cases-feedback cases-feedback--${feedback.type}`}
            ...
            <form className="cases-filter-card" onSubmit={applyFilters}>
                <label className="cases-filter-card__search"><span>Tìm kiếm</span>
                    <div className="cases-search-input"><Search size={15} aria-hidden="true"/><input
                        placeholder="Mã hồ sơ, chủ hồ sơ, thủ tục..." value={draftFilters.search}
                        onChange={(event) => setDraftFilters((current) => ({...current, search: event.target.value}))}/>
                    </div>
                </label>
                <label><span>Trạng thái</span><select value={draftFilters.status}
                ...
                <label><span>Phòng ban</span><input placeholder="Tên phòng ban..." value={draftFilters.department}
                                                    onChange={(event) => setDraftFilters((current) => ({
                                                        ...current,
                                                        department: event.target.value
                                                    }))}/></label>
                <label><span>Người xử lý</span><input placeholder="Tên cán bộ..." value={draftFilters.assignee}
                                                      onChange={(event) => setDraftFilters((current) => ({
                                                          ...current,
                                                          assignee: event.target.value
                                                      }))}/></label>
                <label><span>Từ ngày tiếp nhận</span><input type="date" value={draftFilters.fromDate}
                                                            onChange={(event) => setDraftFilters((current) => ({
                                                                ...current,
                                                                fromDate: event.target.value
                                                            }))}/></label>
                <label><span>Đến ngày tiếp nhận</span><input min={draftFilters.fromDate || undefined} type="date"
            ...
            <article className="cases-table-card">
                <div className="cases-table-card__header"><h2>Danh sách hồ sơ</h2><span>{pagination.total} hồ sơ</span>
                </div>
                        ...
                        <div className="cases-table-wrap">
                            <table className="cases-table">
                                ...
                                {caseList.map((caseItem) => (
                                    <tr className="cases-table__row" key={caseItem.id}
                                        ...
                                        <td>{formatDate(caseItem.receivedAt)}</td>
                                        ...
                                        <td>
                                            <div className="cases-row-actions">
                                                <button aria-label={`Xem ${caseItem.caseCode}`}
                                                        className="cases-action-button" title="Chi tiết" type="button"
                                                        onClick={(event) => {
                                                            event.stopPropagation();
                                                            openCaseDetail(caseItem.id);
                                                        }}><Eye size={14}/></button>
                                                <button aria-label={`Sửa ${caseItem.caseCode}`}
                                                        className="cases-action-button"
                                                        disabled={editLoadingId === caseItem.id} title="Chỉnh sửa"
                                                        type="button" onClick={(event) => {
                                                    event.stopPropagation();
                                                    openEditCase(caseItem.id);
                                                }}>{editLoadingId === caseItem.id ?
                                                    <LoaderCircle className="cases-spinner" size={14}/> :
                                                    <Pencil size={14}/>}</button>
                                                <button aria-label={`Xóa ${caseItem.caseCode}`}
                                                        className="cases-action-button cases-action-button--danger"
                                                        title="Xóa" type="button" onClick={(event) => {
                                                    event.stopPropagation();
                                                    setDeleteError("");
                                ...
                                {!caseList.length && <tr>
                                    <td className="cases-table__empty" colSpan="10">Chưa có dữ liệu hồ sơ.</td>
                                </tr>}
                                </tbody>
                            </table>
                        </div>
                        <div className="cases-pagination">
                            ...
                            <div className="cases-pagination__controls">
                                <button aria-label="Trang trước" disabled={currentPage === 1} type="button"
                                        onClick={() => setCurrentPage((page) => page - 1)}><ChevronLeft
                                    size={16}/> Previous
                                </button>
                                ...
                                <button aria-label="Trang sau" disabled={currentPage === totalPages} type="button"
                                        onClick={() => setCurrentPage((page) => page + 1)}>Next <ChevronRight
                                    size={16}/></button>
                            </div>
                        </div>
                    </>
            ...
            {editingCase &&
                <CaseFormModal initialCase={editingCase} onClose={() => setEditingCase(null)} onSubmit={handleUpdate}/>}
            {deletingCase &&
                <DeleteCaseModal caseItem={deletingCase} deleting={deleting} error={deleteError} onClose={() => {
                    setDeletingCase(null);
                    setDeleteError("");
