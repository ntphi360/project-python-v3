...
function NotificationsPage() {
    ...
    return (
        <section className="notifications-page">
            ...
            {!loading && error &&
                <div className="notifications-state notifications-state--error" role="alert">{error}</div>}
            ...
            {!loading && items.length > 0 && (
                <div className="notifications-list">
                    {items.map((item) => (
                        <article className={`notification-item${item.isRead ? " notification-item--read" : ""}`}
                                 key={item.id}>
                            ...
                            {!item.isRead && (
                                <button disabled={markingId === item.id} type="button"
                                        onClick={() => handleMarkRead(item.id)}>
                                    {markingId === item.id ? <LoaderCircle className="cases-spinner" size={14}/> :
                                        <Check size={14}/>}
                                    Đã đọc
                                </button>
                            )}
