...
const reminderChannelLabels = Object.fromEntries(
  reminderChannels.map((channel) => [channel.value, channel.label])
);
...
function AlertBadge({ alertType, label }) {
  if (!alertType) return "—";

  return (
    <span className={`alert-badge alert-badge--${getAlertKey(alertType)}`}>
      {label}
    </span>
  );
}
...
function buildDefaultReminder(caseItem) {
  ...
  if (caseItem.alertType === "OVERDUE") {
    return `Hồ sơ ${caseItem.caseCode} đã ${caseItem.remainingText.toLocaleLowerCase("vi")}. Vui lòng kiểm tra và xử lý hồ sơ sớm nhất có thể.`;
  }

  return `Hồ sơ ${caseItem.caseCode} - ${caseItem.caseName ?? "Chưa có tên"} còn ${caseItem.remainingText.replace(/^Còn\s+/i, "").toLocaleLowerCase("vi")} trước hạn xử lý. Vui lòng kiểm tra và xử lý hồ sơ đúng tiến độ.`;
}

function ReminderModal({ caseItem, onClose, onSent }) {
  const [message, setMessage] = useState(() => buildDefaultReminder(caseItem));
  const [channels, setChannels] = useState(["SYSTEM"]);
  const [error, setError] = useState("");
  const [submitting, setSubmitting] = useState(false);
  const submittingRef = useRef(false);
  const allChannelsSelected = channels.length === reminderChannels.length;

  function toggleAllChannels() {
    setChannels(
      allChannelsSelected
        ? []
        : reminderChannels.map((channel) => channel.value)
    );
    setError("");
  }
  ...
  async function handleSubmit(event) {
    ...
    try {
      const result = await sendCaseReminder(
        caseItem.caseId,
        normalizedMessage,
        channels,
      );
      ...
      if (!successfulChannels.length) {
        const failureMessage = Object.entries(channelResults)
          .map(([channel, channelResult]) => (
            `${reminderChannelLabels[channel] ?? channel}: ${channelResult.error}`
          ))
          .join("; ");
        setError(failureMessage || "Không gửi được nhắc nhở qua các kênh đã chọn.");
        return;
      }

      onSent(caseItem, channelResults);
    } catch (requestError) {
      ...
      <form className="reminder-form" onSubmit={handleSubmit}>
        <div className="case-modal__body reminder-form__body">
          <dl className="reminder-details">
            ...
            <div><dt>Chủ hồ sơ</dt><dd>{caseItem.caseName ?? "—"}</dd></div>
            <div><dt>Lĩnh vực</dt><dd>{caseItem.fieldName ?? "—"}</dd></div>
            <div><dt>Thủ tục hành chính</dt><dd>{caseItem.procedureName ?? "—"}</dd></div>
            <div><dt>Người xử lý</dt><dd>{caseItem.assigneeName ?? "Chưa phân công"}</dd></div>
            <div><dt>Phòng ban</dt><dd>{caseItem.departmentName ?? "—"}</dd></div>
            <div><dt>Email</dt><dd>{caseItem.assigneeEmail ?? "—"}</dd></div>
            <div><dt>Số điện thoại</dt><dd>{caseItem.assigneePhone ?? "—"}</dd></div>
            <div><dt>Hạn xử lý</dt><dd>{formatDateTime(caseItem.dueAt)}</dd></div>
            <div><dt>Thời hạn</dt><dd className={`alerts-countdown alerts-countdown--${getAlertKey(caseItem.alertType)}`}>{caseItem.remainingText}</dd></div>
            <div><dt>Mức cảnh báo</dt><dd><AlertBadge alertType={caseItem.alertType} label={caseItem.alertLabel} /></dd></div>
          </dl>
        ...
        <footer className="case-modal__footer">
          ...
          <button
            ...
            disabled={submitting || !message.trim() || !channels.length}
            type="submit"
          >
            {submitting
              ? <LoaderCircle className="cases-spinner" size={14} />
              : <Send size={14} />}
            {submitting ? "Đang gửi..." : "Gửi nhắc nhở"}
          </button>
        </footer>
      </form>
    </CaseModal>
...
function AlertsPage() {
  ...
  const kpiCards = [
    { label: "Tổng cảnh báo", value: summary.total, icon: BellRing, tone: "blue" },
    { label: "Sắp hạn", value: summary.nearDue, icon: Clock3, tone: "orange" },
    { label: "Đến hạn hôm nay", value: summary.dueToday, icon: CalendarClock, tone: "purple" },
    { label: "Quá hạn", value: summary.overdue, icon: TriangleAlert, tone: "red" },
  ];
  ...
  return (
    <section className="alerts-page">
      <div className="alerts-page__heading">
        ...
        <p>Theo dõi hồ sơ sắp hạn, đến hạn và quá hạn xử lý.</p>
      </div>
      ...
      <form className="cases-filter-card alerts-filter-card" onSubmit={applyFilters}>
        <label>
          ...
          <div className="cases-search-input">
            <Search size={15} />
            <input placeholder="Mã hoặc chủ hồ sơ" value={draftFilters.keyword} onChange={(event) => changeDraftFilter("keyword", event.target.value)} />
          </div>
        </label>
        <label>
          <span>Loại cảnh báo</span>
          <select value={draftFilters.alertType} onChange={(event) => changeDraftFilter("alertType", event.target.value)}>
            <option value="">Tất cả cảnh báo</option>
            {alertTypes.map((alertType) => <option key={alertType.value} value={alertType.value}>{alertType.label}</option>)}
          </select>
        </label>
        <label><span>Phòng ban</span><input placeholder="Tên phòng ban" value={draftFilters.department} onChange={(event) => changeDraftFilter("department", event.target.value)} /></label>
        <label><span>Người xử lý</span><input placeholder="Tên cán bộ" value={draftFilters.assignee} onChange={(event) => changeDraftFilter("assignee", event.target.value)} /></label>
      ...
      <article className="cases-table-card alerts-table-card">
        <div className="cases-table-card__header"><h2>Danh sách cảnh báo</h2><span>{pagination.totalItems} hồ sơ</span></div>
        {loading && <div className="cases-table__empty">Đang tải dữ liệu cảnh báo...</div>}
          ...
          <>
            <div className="cases-table-wrap">
              <table className="cases-table alerts-table">
                <thead><tr><th>Mã hồ sơ</th><th>Chủ hồ sơ</th><th>Thủ tục hành chính</th><th>Phòng ban</th><th>Người xử lý</th><th>Hạn xử lý</th><th>Ngày hẹn trả</th><th>Thời hạn</th><th>Mức cảnh báo</th><th>Thao tác</th></tr></thead>
                <tbody>
                  {alertCases.map((caseItem) => (
                    <tr className={`alerts-row alerts-row--${getAlertKey(caseItem.alertType)}`} key={caseItem.caseId}>
                  ...
                  {!alertCases.length && <tr><td className="cases-table__empty" colSpan="10">Không có hồ sơ cảnh báo phù hợp.</td></tr>}
                </tbody>
              </table>
            </div>
            <div className="cases-pagination">
              ...
              <div>
                <button aria-label="Trang trước" disabled={!pagination.hasPrev} type="button" onClick={() => setCurrentPage((page) => page - 1)}><ChevronLeft size={16} /> Previous</button>
                <button aria-label="Trang sau" disabled={!pagination.hasNext} type="button" onClick={() => setCurrentPage((page) => page + 1)}>Next <ChevronRight size={16} /></button>
              </div>
            </div>
          </>
        )}
      ...
      {reminderCase && (
        <ReminderModal
          caseItem={reminderCase}
          onClose={() => setReminderCase(null)}
          onSent={(caseItem, results) => {
            const successful = Object.entries(results)
              .filter(([, result]) => result.success)
              .map(([channel]) => reminderChannelLabels[channel] ?? channel);
            const failed = Object.entries(results)
              .filter(([, result]) => !result.success)
              .map(([channel, result]) => (
                `${reminderChannelLabels[channel] ?? channel} (${result.error})`
              ));
            ...
            setFeedback({
              ...
              message: failed.length
                ? `Hồ sơ ${caseItem.caseCode}: đã gửi qua ${successful.join(", ")}; không gửi được qua ${failed.join(", ")}.`
                : `Đã gửi nhắc nhở hồ sơ ${caseItem.caseCode} qua ${successful.join(", ")} cho ${caseItem.assigneeName}.`,
            });
          }}
        />
      )}
